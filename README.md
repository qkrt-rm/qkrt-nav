...
2. **Odometry drift on pure translation** — The `GEAR_RATIO` of 9.75 was empirically tuned for straight-line travel. However, sometimes during translation, lidar scans physically drift and smear. The current perceived solution is to tune the odometry better
...
- **Fix base rotation → turret drift bug** — The core localization correctness issue. The most recent idea is to compare `pose.pose.orientation.w` in comm_hub.py with `turret_data[0] #angle between chassis and turret`, and correct the value `pose.pose.orientation.w`, which is what ends up getting published to `/odom`. The angular odometry is not great, however the yaw value from the gimbal encoder is highly accurate. The issue is that turret rotation also affects this value, since `turret_data[0]` is the angle between the **chassis relative to the turret**. The true chassis heading should be something like `turret_data[0] - gyro_z * dt`, and has yet to be implemented in `turret_joint_state_publisher`. 

Consider merging `turret_joint_state_publisher` into `comm_hub.py` to be able to use the code in the former to modify `pose.pose.orientation.z` in the latter.

- **Validate and tune odometry calibration** — The `GEAR_RATIO = 9.75` was set from a single 1 m test. Run a formal calibration across multiple distances and rotation speeds. Check `WHEEL_BASE_X/Y` against physical measurements on the actual robot.

- **End-to-end autonomous navigation test** — Send a 2D Nav goal via RViz or action client and verify the robot reaches it without oscillation or map corruption. Current state: localization works well enough**, navigation stack launches, but closed-loop nav has not been fully validated on hardware.

- **Behaviour Tree Creation and Tuning** - create a working behaviour tree and tune it to work between two arbitrary points

- **Verify NAV stack works as Expected**

- **Test/Attempt to use the new Sentry_Vision package** - Test cv localization in tandem with the other forms of localization

- **Test AutoNav and AutoAim Minimum Viable Product** - Autonav commands send the robot to the middle, robot camps in mid. No Nav commands sent. Auto Aim activates, and shoots other Robots. Autonav starts resending nav commands when health/battery/balls are low.
