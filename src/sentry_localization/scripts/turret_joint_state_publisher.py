...
from nav_msgs.msg import Odometry # for reading base angular velocity to subtract from gyro for better turret position estimation
import math
...
class TurretJointStatePublisher(Node):
    ...
    def __init__(self):
        ...
        self.odom_subscriber_ = self.create_subscription(Odometry, '/odom', self.odom_cb, 10) # for reading base angular velocity to subtract from gyro for better turret position estimation
        self.gyro_z = 0.0
        self.odom_omega = 0.0 # base angular velocity around vertical axis from odometry, used to subtract from gyro for better turret position estimation
    ...
    def imu_cb(self, msg: Imu):
        ...
        if self.prev_imu_time is not None:
            ...
            self.integrated_turret_position += (self.gyro_z - self.odom_omega) * dt 
