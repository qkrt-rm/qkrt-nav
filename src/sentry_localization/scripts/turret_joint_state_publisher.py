...
class TurretJointStatePublisher(Node):
    ...
    def publish_joint_states(self, turret_data):
        ...
        # gimbal_joint: turret angle relative to chassis (base_link).
        # turret_data[0] is turretYawPos. Sign is negated because turretYawPos
        # increases when the chassis rotates CCW (turret appears CW relative to chassis),
        # but gimbal_joint CCW-positive means we need the opposite sign.
        js_msg.name = [
            'gimbal_joint',
            'turret_shaft_joint',
    ...
    def _timer_cb(self):
        ...
        dt = min(dt, 0.5)
        # Extrapolate raw encoder position; publish_joint_states negates it.
